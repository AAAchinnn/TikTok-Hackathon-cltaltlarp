...
import optimized_transformer as opt_mod
...
def one_layer_diagnostics(baseline, optimized, x, valid_mask, causal, atol, rtol):
    ...
    with torch.inference_mode():
        ...
        # Compare only legal score entries; masked -inf entries are excluded.
...
def main() -> int:
    ...
    if device.type == "cuda" and importlib.util.find_spec("triton") is not None:
        try:
            ...
            print(f"Triton attention smoke-test output shape={tuple(out.shape)}")
            print("Triton path is callable.")
        except Exception as exc:
            print("Triton path smoke-test FAILED:")
            print(repr(exc))
