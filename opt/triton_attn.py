"""PARKED -- not on the shipped path.

Moved here verbatim from achin/optimized_transformer.py so the work is not
lost. Nothing imports it: opt/__init__.py deliberately leaves it out, so it
costs nothing at runtime and cannot break a benchmark run.

To make it live, register it as a dispatcher candidate
(@register("attn_block", ...) in opt/dispatcher.py) and measure it against the
general block behind the correctness gate. Before spending that time: the
general block delegates to SDPA, which on sm75 means the mem-efficient kernel,
and a hand-written Triton kernel has to beat that. The project's own planning
notes rated this the lowest-value item on the list.
